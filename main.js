...
// Q1. How many total commits were made in all of steve's events?
// Sorry greg the only way I could understand was to write it out 3 times
let commitsTotal = 0

// for (let i = 0; i <githubData.length; i++) {
//   const myArray = githubData[i]
//   if (myArray.payload.hasOwnProperty("commits")) {
//       commitsTotal = commitsTotal + myArray.payload.commits.length
//   }
// }

// console.log(commitsTotal)

// for (let i = 0; i <githubData.length; i++) {
//   if (githubData[i].payload.hasOwnProperty("commits")) {
//       commitsTotal += githubData[i].payload.commits.length
//   }
// }

// console.log(commitsTotal)

// loop through outer array, in each objects find the commits array and grab its length

githubData.forEach((myArray) => {
  if (myArray.payload.hasOwnProperty("commits")) {
    commitsTotal += myArray.payload.commits.length
  }
})

console.log("totalCommits", commitsTotal)

// // Q2. How many of each event type are there? (PullRequestEvent, PushEvent, etc)
let typeArray = []

githubData.forEach((myArray) => {
  typeArray.push(myArray.type)
})
...
typeArray.forEach((keys) => {
  countObj[keys] = (countObj[keys] || 0) + 1
})
console.log(countObj)

// Q3. List all Github users who submitted a pull request that was approved by Steve.
...
// Q4. List all repositories on which Steve had an event, and show how many events were on each one.

let repoArray = []

githubData.forEach((myArray) => {
  repoArray.push(myArray.repo.name)
})
...
repoArray.forEach((keys) => {
  repoCount[keys] = (repoCount[keys] || 0) + 1
})
console.log("repocount", repoCount)

// Q5. Which event had the most number of commits?
let idName = ""
let commitsCount = 0

githubData.forEach((myArray) => {
  if (myArray.payload.hasOwnProperty("commits")) {
    if (myArray.payload.commits.length > commitsCount)
      commitsCount = myArray.payload.commits.length
    idName = myArray.id
  }
  return idName
})

console.log("idName", `${idName}: ${commitsCount}`)

// Q6 Which programming langugages were affected by Steve's events?
let langArray = [];


githubData.forEach((myArray) => {
  if (myArray.type === "PullRequestEvent") {
    langArray.push(myArray.payload.pull_request.head.repo.language)
  }
});
